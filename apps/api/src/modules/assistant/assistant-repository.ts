...
export function createAssistantRepository(prisma: PrismaClient) {
  return {
    async schedule(input: { workspaceId: string; conversationId: string; trigger: 'manual' | 'inbound'; messageId?: string; actorUserId?: string; instruction?: string }) {
      return prisma.$transaction(async tx => {
        ...
        const latest = await tx.message.findFirst({ where: { workspaceId: input.workspaceId, conversationId: input.conversationId, type: { notIn: ['internal_note', 'system'] } }, orderBy: [{ createdAt: 'desc' }, { id: 'desc' }] });
        ...
        const data = {
          ...
          leaseToken: null, leaseUntil: null, lastError: null, attempts: 0,
    ...
    async invalidate(workspaceId: string, conversationId: string, status = 'stale') {
      await prisma.assistantConversationState.updateMany({ where: { workspaceId, conversationId }, data: { status, revision: { increment: 1 }, leaseToken: null, leaseUntil: null, scheduledAt: null } });
    },
    async due(now = new Date()) {
      ...
      return prisma.assistantConversationState.findMany({ where: { status: 'pending', scheduledAt: { lte: now } }, orderBy: { scheduledAt: 'asc' }, take: 2 });
    },
