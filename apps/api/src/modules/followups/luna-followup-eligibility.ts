...
import { preScreenFollowupCandidate, type FollowupEligibility } from "./jev-followup-eligibility.js";
...
const SYSTEM_PROMPT = [
  ...
  "Uma mensagem proativa da empresa sem resposta anterior do cliente ainda não é uma conversa que esfriou. Não agende sequência para tabela de preços, anúncio ou pergunta de prospecção sem envolvimento prévio do cliente.",
...
export function createLunaFollowupEligibility(input: {
  ...
  return {
    async evaluate(candidate) {
      const prescreened = preScreenFollowupCandidate(candidate);
      ...
      if (!messages.some((message) => message.id === candidate.anchorMessageId)) {
