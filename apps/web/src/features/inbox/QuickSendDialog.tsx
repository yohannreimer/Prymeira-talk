...
export function QuickSendDialog({ channels, initialChannelId, getToken, onClose, onSent }: {
  ...
  const recentIds = new Set(recentContacts.map((contact) => contact.id));
  const visibleMatches = query ? matches : matches.filter((contact) => !recentIds.has(contact.id));
    ...
    <section className="inbox-action-dialog inbox-action-dialog--wide" role="dialog" aria-modal="true" aria-labelledby="quick-send-title">
      ...
      <div className="inbox-action-dialog-body">
        {campaignId && progress ? <div className="inbox-bulk-results" role="status">
          ...
          {!reviewing ? <>
            <label className="inbox-field-label" htmlFor="quick-recipient-search">Escolher contatos · {recipients.length} selecionados</label>
            <div className="inbox-dialog-search"><Search size={16} /><input id="quick-recipient-search" type="search" placeholder="Buscar nome ou telefone" value={query} onChange={(event) => { setQuery(event.target.value); setMatches([]); setNextCursor(null); setError(null); }} /></div>
            {!query && recentContacts.length ? <><span className="inbox-picker-section-label">Conversas recentes</span><div className="inbox-contact-picker inbox-contact-picker--recent" role="listbox" aria-multiselectable="true" aria-label="Conversas recentes para envio">{recentContacts.map((contact) => {
              const checked = recipients.some((recipient) => recipient.phone === normalizeRecipientPhone(contact.phone));
              return <button key={contact.id} type="button" role="option" aria-selected={checked} onClick={() => toggleRecipient(contact)}><ContactAvatar contactId={contact.id} name={contact.name || contact.phone} className="inbox-picker-avatar" /><span className="inbox-picker-label"><strong>{contact.name?.trim() || contact.phone}</strong><small>{contact.phone}</small></span><span className={`inbox-picker-check${checked ? ' is-selected' : ''}`}>{checked ? <Check size={14} /> : null}</span></button>;
            })}</div></> : null}
            <span className="inbox-picker-section-label">{query ? 'Resultados da busca' : 'Todos os contatos'}</span>
            <div className="inbox-contact-picker" role="listbox" aria-multiselectable="true" aria-label="Contatos para envio">
              {loadingContacts ? <p className="list-note">Carregando contatos...</p> : visibleMatches.length === 0 && !nextCursor ? <p className="list-note">Nenhum outro contato encontrado.</p> : visibleMatches.map((contact) => {
                const phone = normalizeRecipientPhone(contact.phone);
                const checked = recipients.some((recipient) => recipient.phone === phone);
                return <button key={contact.id} type="button" role="option" aria-selected={checked} onClick={() => toggleRecipient(contact)}>
            ...
            {recipients.length ? <div className="inbox-recipient-chips">{recipients.map((recipient) => <span key={recipient.phone}>{recipient.name}<button type="button" aria-label={`Remover ${recipient.name}`} onClick={() => setRecipients((current) => current.filter((item) => item.phone !== recipient.phone))}><X size={13} /></button></span>)}</div> : null}
