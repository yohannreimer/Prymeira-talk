...
- Para cada telefone com DDI, preservar a sequência de dígitos exibida no lead como candidata primária. Números brasileiros locais reconhecíveis, com DDD e 10 ou 11 dígitos, recebem `55` antes da consulta; a exibição do lead não muda. `normalizedPhone` pode servir para agrupar registros, mas não deve substituir silenciosamente o número enviado à Evolution quando o original está disponível.
...
- Ao repetir um job parcialmente falho, gerar `numbers`, `lookups` e `entries` somente para os telefones falhos, preservando suas candidatas originais. Jobs legados sem `lookups` derivam as candidatas a partir do telefone salvo.
...
- Um lead do Google Maps com `(47) 99139-6920` envia o mesmo número completo `5547991396920`, enquanto um telefone fixo local recebe `55` sem criar uma variante móvel.
